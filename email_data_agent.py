...
class EmailDataAgent:
    ...
    def load_data(self, data_file: str) -> bool:
        ...
        try:
            ...
            self.email_by_id = {email["message_id"]: email for email in self.emails}
    ...
    def _llm_correlate(self, activity: Dict[str, Any], email: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Cache result
            cache_key = self._get_cache_key(activity.get("id", "unknown"), email.get("message_id", "unknown"))
...
"""
    ...
    def _parse_correlation_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            related_match = re.search(r'RELATED:\s*(Yes|No)', response, re.IGNORECASE)
            if related_match:
    ...
    def _filter_candidate_emails(self, activity: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            activity_date = datetime.strptime(f"{activity_date_str} {current_year}", "%b %d %Y")
        except (ValueError, TypeError):
        ...
        for email in self.emails:
            ...
            email_course = email.get("course_context", "").lower()
    ...
    def find_correlations(self, activity: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if not candidates:
            logger.info(f"No candidate emails found for activity: {activity.get('Title', '')}")
            return []
        
        logger.info(f"Found {len(candidates)} candidate emails for correlation")
        
        # Perform LLM correlation for each candidate
        correlated_emails = []
        for email in candidates:
            # Check cache first
            cache_key = self._get_cache_key(activity_id, email.get("message_id", "unknown"))
            if cache_key in self.correlation_cache:
        ...
        correlated_emails.sort(key=lambda x: x["correlation"]["confidence"], reverse=True)
    ...
    def start_background_correlation(self, activities: List[Dict[str, Any]]) -> None:
        ...
        logger.info(f"Started background correlation for {len(activities)} activities")
    ...
    def _background_correlation_worker(self, activities: List[Dict[str, Any]]) -> None:
        ...
        try:
            for activity in activities:
                ...
                logger.debug(f"Found {len(correlated)} correlated emails for activity {activity_id}")
